#!/usr/bin/env perl
#This script enable to convert a sra to fastq. it uses a file with input data called list_file_single_end.conf.
# list_file_single_end.conf format is: sra-file
...
my $cmd = "fastq-dump --outdir $out_path $file_path";
system($cmd);